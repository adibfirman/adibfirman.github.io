...
export const MarkdownParser: Components = {
  // Headings
  h1: ({ node, ...props }) => (
    <h1
      {...props}
  ...
  ol: ({ node, ...props }) => (
    <ol {...props} className="list-decimal pl-6 space-y-1 text-lg leading-7" />
  ),
  li: ({ node, ...props }) => (
  ...
  hr: ({ node, ...props }) => (
    <hr {...props} className="h-1 my-4 bg-mystic-soft/50 rounded-sm border-0" />
  ),
